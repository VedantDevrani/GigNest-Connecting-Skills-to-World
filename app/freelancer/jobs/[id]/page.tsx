...
export default function JobDetailsPage() {
    ...
    const { data: job, loading, error, refetch } = useCachedFetch<any>(
        `freelancer-job-${jobId}`,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch('/api/freelancer/proposals', {
                ...
                body: JSON.stringify({ jobId, coverLetter, bidAmount }),
            });
            ...
            setFormSuccess(true);
            invalidateCache(`freelancer-job-${jobId}`);
            await refetch(true);
        } catch (err: unknown) {
    ...
    const handleWithdraw = async (proposalId: string) => {
        ...
        setFormLoading(true);
        try {
            const res = await fetch(`/api/freelancer/proposals/${proposalId}`, { method: 'DELETE' });
    ...
    if (loading && !job) return <PageLoader text="Loading job details..." />;
    ...
    return (
        <div className="space-y-6 max-w-5xl mx-auto">
            <Link href="/freelancer/jobs" prefetch className="inline-flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-primary transition-colors">
                ...
                <div className="flex-1 w-full space-y-6">
                    <Card className="p-8 bg-white dark:bg-gray-900 border-gray-100 dark:border-gray-800 shadow-sm">
                        ...
                        <div className="prose dark:prose-invert max-w-none">
                            ...
                            <p className="text-gray-600 dark:text-gray-300 leading-relaxed whitespace-pre-wrap mb-8">{job.description}</p>
                        </div>
                        <div>
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white font-poppins mb-3">Skills & Expertise Required</h3>
                            <div className="flex flex-wrap gap-2">
                                {job.skills.map((skill: string) => (
                                    <span key={skill} className="px-3 py-1.5 text-sm font-medium bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-lg">{skill}</span>
                                ))}
                            </div>
                        </div>
                    </Card>
