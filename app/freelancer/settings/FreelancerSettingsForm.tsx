...
export function FreelancerSettingsForm({ initialProfile }: { initialProfile: Profile }) {
    ...
    const [formData, setFormData] = useState({
        ...
        hourlyRate: initialProfile.hourlyRate?.toString() || '',
                ...
                <Card className="col-span-1 md:col-span-2 p-6 bg-white dark:bg-gray-900 shadow-sm border-gray-100 dark:border-gray-800">
                    <form onSubmit={handleSave} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <Input label="Hourly Rate ($/hr)" type="number" name="hourlyRate" placeholder="e.g. 50" value={formData.hourlyRate} onChange={handleChange} />
                        </div>
                        ...
                        <div className="space-y-3 pt-2">
                            ...
                            <div className="flex flex-wrap gap-2 mb-3">
                                {formData.skills.map((skill) => (
                                    <div key={skill} className="flex items-center gap-2 px-3 py-1.5 bg-primary/10 text-primary dark:bg-primary/20 rounded-full text-sm font-medium">
                                        {skill}
                                        <button type="button" onClick={() => handleRemoveSkill(skill)} className="hover:bg-primary/20 dark:hover:bg-primary/30 rounded-full p-0.5 transition-colors">
                                            <X className="w-3 h-3" />
                                        </button>
                                    </div>
                                ))}
                            ...
                            <div className="flex gap-2">
                                <input
                                    ...
                                    className="flex-1 px-4 py-2.5 bg-gray-50 dark:bg-[#111827] border border-gray-200 dark:border-gray-800 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400"
