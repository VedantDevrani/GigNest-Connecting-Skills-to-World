...
import { Briefcase, CreditCard, Calendar, XCircle, CheckCircle2, Trash2 } from 'lucide-react';
...
export function ClientJobsList({ initialJobs }: { initialJobs: ClientJob[] }) {
    ...
    const handleToggleStatus = async (id: string, currentStatus: string) => {
        ...
        try {
            ...
            setJobs(jobs.map(job => job.id === id ? { ...job, status: newStatus as ClientJob['status'] } : job));
            invalidateCache('client-jobs');
            router.refresh();
        } catch (err: unknown) {
    ...
    return (
        <div className="space-y-6 max-w-5xl">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                ...
                <Link href="/client/jobs/create" prefetch>
                    <Button className="px-6 py-2">+ Post a New Job</Button>
                </Link>
            </div>
                    ...
                    <AnimatePresence>
                        {jobs.map((job) => (
                            <motion.div
                                ...
                                className="w-full"
                            >
                                <Card className="p-6 bg-white dark:bg-gray-900 shadow-sm border-gray-100 dark:border-gray-800 hover:shadow-md transition-shadow">
